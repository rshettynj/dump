...
def uploadFile(awsroleArn,imagename) {
                ...
                def out = sh(returnStdout: true, script: "AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID} AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} AWS_SECURITY_TOKEN=${AWS_SECURITY_TOKEN} /usr/bin/aws --region us-east-1 ecr get-login --no-include-email")
                }
                else
                {
                def out = sh(returnStdout: true, script: "/usr/bin/aws --region us-east-1 ecr get-login --no-include-email")
                }
